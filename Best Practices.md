...
Bastion hosts are part of a broken security model developed by people who never had to use any software in production, much less develop anything.  The result is they are an unusable model requiring terminal sessions and more to hop between multiple machines.  As a general rule we don't put them in a Quick Start.  There would have to be a very good reason to break this rule.
