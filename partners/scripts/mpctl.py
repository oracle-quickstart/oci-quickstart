...
class Artifact:
    ...
    def __init__(self, resource):
        ...
        for resourceProperty in resource["properties"]:
            ...
            args.artifactId = int(resourceProperty["value"])
            bind_action_dic()
            uri, r = do_get_action()
...
def do_update_stack():
    ...
    apicall = action_api_uri_dic[args.action]
    uri = api_url + apicall
    api_headers = {'Content-Type': 'multipart/form-data', 'boundary': '----WebKitFormBoundary7MA4YWxkTrZu0gW',
                   'charset': 'UTF-8',
                   'X-Oracle-UserId': creds['user_email'], 'Authorization': f"Bearer {access_token}"}
    body = '{"version": "' + args.version_string + '", "description": "Description of package", "tncId": "' + str(
        tcnId) + '", "serviceType": "OCIOrchestration"}'
    body_json = json.loads(body)
